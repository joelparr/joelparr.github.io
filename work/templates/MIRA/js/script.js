...
{ 
	// Enable rolldown menu	
	$('.rolldown-trigger a').toggle(function(){
		$('.rolldown-trigger').addClass('rolldown-expanded').removeClass('rolldown-trigger').unbind('mouseenter, mouseleave');
		$('.rolldown-expanded').fadeBorderColor({fadeInColor: '#ffffff', fadeOutColor: '#ffffff', duration: 250});
		$('.rolldown-expanded').moveBackgroundPosition({childSelector: 'a', fadeInPosition: '0 40%', fadeOutPosition: '0 60%', duration: 100});
		$('#post-header-container').stop().animate({top: '194px'}, 500, function(){
			$('#rolldown').stop().fadeIn(300);
		});
		return false;
	}, function(){
		$('#rolldown').stop().fadeOut(300, function(){
			$('#post-header-container').stop().animate({top: '0px'}, 500, function(){
				$('.rolldown-expanded').addClass('rolldown-trigger').removeClass('rolldown-expanded').unbind('mouseenter, mouseleave');
				$('.rolldown-trigger').css('border-color', '#01a4a6');
	...
	$('.logged-in').moveBackgroundPosition({fadeInPosition: '15px -41px', fadeOutPosition: '15px -39px', duration: 100});
	$('.pager .first a').moveBackgroundPosition({fadeInPosition: '43% 8px', fadeOutPosition: '50% 8px', duration: 100});
	$('.pager .last a').moveBackgroundPosition({fadeInPosition: '60% 8px', fadeOutPosition: '50% 8px', duration: 100});
	...
	$('.vertical a').addPadding({childSelector: 'span time, span b', boxSide: 'right', moveInPadding: '11px', moveOutPadding: '8px', duration: 100});	
});